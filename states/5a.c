...
void execute_5a() {
  ...
  for (uint16_t i = 0; i < array_5_size; i+=3) {
    for (uint8_t j = 0; j < input_array_5[i]; j++) {
      uint16_t to_index = (input_array_5[i+2]-1)*max_stack_size_5+input_array_5_stack_counts_mutable[input_array_5[i+2]-1];
      ...
      input_array_5_stack_counts_mutable[input_array_5[i+2]-1]++;
